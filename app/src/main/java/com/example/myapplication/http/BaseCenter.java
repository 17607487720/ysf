...
public class BaseCenter {
    ...
    public static <T> Observable<BaseEntity<T>> cacheRequest(BaseRequest request, Class<T> tClass) {
        return RxNetOnSubscribe.create(request.getCall(), tClass).enableCache(true).toRxMain();
    }

    public static <T> Observable<BaseEntity<T>> cacheRequest(BaseRequest request, Class<T> tClass, boolean keepCache, boolean takeCache) {
